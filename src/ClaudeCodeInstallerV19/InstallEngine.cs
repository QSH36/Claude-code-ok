...
public class InstallEngine
{
    ...
    {
        _log("═══════════════════════════════════");
        _log($"  Claude Code Installer v1.0.6 [{(_s.IsSimple ? "小白" : "专业")}] | Shimizu");
        ...
        try
        {
            ...
            if (!_s.GitOk)
            {
                _log("  Git 安装包约 60MB，预计需 2-5 分钟，请去喝杯咖啡吧~");
                _log("  安装过程中如弹出 [更改硬盘驱动器] 或 UAC 提示，请点击 [是]");
            ...
            if (_s.InstallTools)
            {
                ...
                step++; _progress(step, total);
                _log($"[{step}/{total}] Python 依赖安装  需约 5-8 分钟，请耐心等待...");
                _log("  (mss + pytesseract + pyautogui + pillow + pygetwindow + playwright ≈ 200MB)");
                await InstallPy();
